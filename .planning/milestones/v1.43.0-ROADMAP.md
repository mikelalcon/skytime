...
- [x] 07.1-06-PLAN.md — pkg/cli/server.go listener bind + listener-first shutdown + reasonable HTTP defaults
- [x] 07.1-07-PLAN.md — examples/http-github-webhook/webhook_demo.star crash-recovery demo flow + trigger
- [x] 07.1-08-PLAN.md — walkthrough docs (5min README + full docs/walkthroughs/) + firewall extension + headings gate
**UI hint**: no
...
- [x] 07.3-01-PLAN.md — executeFlow extraction (UI-04): `flowlaunch.Execute` + `flowlaunch.BuildWorkflowInput` shared seam (Open Q 1 Option 1) + AST call-site firewall
...
- [x] 07.3-04-PLAN.md — HTTP handlers + //go:embed template + SSE wiring + pkg/cli/server.go boot integration + 2 new CLI flags + first browser UAT checkpoint
- [x] 07.3-05-PLAN.md — docs/walkthroughs/dashboard.md (12 H2 sections) + dashboard-smoke.sh + extended TestServerCmd_DrainOnSIGTERM (SSE shutdown delivery) + headings firewall + final browser UAT
**UI hint**: yes
...
- [x] 07.4-04-PLAN.md — Collapse extbin/main.go to ≤30 lines; move build-your-own-binary teaching block to pkg/cli/doc.go (CLI-12, D-7.4-13, D-7.4-14)
...
- [x] 07.5-03-snippet-aws-irsa-PLAN.md — aws.go (newAWSCredentials via AWS Secrets Manager + IRSA) + AWS section in temporal-auth.md + drift-test row (AUTH-02)
...
- [x] 07.5-06-readme-and-project-edits-PLAN.md — README.md doc-index link + for-extension-developers/README.md sub-index + PROJECT.md verbatim Web UI / dashboard Out-of-Scope carve-out (D-7.5-10)
