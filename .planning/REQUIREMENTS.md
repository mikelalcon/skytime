...
- [ ] **TRIG-02**: Extensions can return a new `TriggerSource` value type (parallel to `ActionRef` for ops). The parser stores `TriggerSource` as an opaque payload on the `dag.Trigger` node; the runtime unpacks it via type-switch.
- [ ] **TRIG-03**: A new `dag.Trigger` node type with stable JSON marshaling (`Kind`, `FlowName`, `Source`, `MapLambda`, `IdempotencyLambda`, `CredentialID`). Trigger nodes serialize/deserialize round-trip.
...
- [ ] **AUTH-01**: `docs/for-extension-developers/temporal-auth.md` ships with a working WIF → Google Secret Manager → Temporal Cloud snippet (Go code). Customer's GCP workload identity issues short-lived tokens, secret manager hands a Temporal Cloud API key, `client.Credentials` rotates on a refresh callback.
- [ ] **AUTH-02**: An IRSA → AWS Secrets Manager → Temporal Cloud snippet in temporal-auth.md. Customer's k8s service account assumes an IAM role, secret manager hands the Temporal credential, rotation handled the same way.
